...
function DetailedPost() {
    ...
    useEffect(() => {
        const fetchInteractionsData = async () => {
            try {
                const response = await fetch(`http://localhost:5000/post/${id}/interactions/${userInfo?.username}`);
                const data = await response.json();
    ...
    const handleUpvote = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/post/${id}/upvote`, {
                method: 'PUT',
    ...
    const handleDownvote = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/post/${id}/downvote`, {
                method: 'PUT',
                body: JSON.stringify({ user: userInfo?.username }),
                headers: {
                    'Content-Type': 'application/json',
    ...
    const handleSave = async () => {
        ...
        if (post) {
            try {
                const response = await fetch(`http://localhost:5000/post/${id}/save`, {
                    method: 'PUT',
    ...
    const addComment = async (comment) => {
        ...
        try {
            ...
            if (response.ok) {
                setCommentsData(data);
                await fetch(`http://localhost:5000/post/${id}/notify`, {
                    method: 'PUT',
