...
function CommentComp({ cmt, post }) {
  ...
  useEffect(() => {
    const fetchInteractionsData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/post/${post._id}/comment/${cmt._id}/cominteractions/${userInfo?.username}`);
  ...
  const handleUpvote = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/post/${post._id}/comment/${cmt._id}/upvote`, {
        method: 'PUT',
  ...
  const handleDownvote = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/post/${post._id}/comment/${cmt._id}/downvote`, {
        method: 'PUT',
        body: JSON.stringify({ user: userInfo?.username }),
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/post/${post._id}/save`, {
        method: 'PUT',
